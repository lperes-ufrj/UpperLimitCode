...
for i in range(9,12): #Each BDM sample gamma and mass value 
   ...
   #     poi = np.logspace(-7.,-6.,num=900, base=10)
    ...
    ################################################################
        ...
        if (np.round(per95_signal_bkg,1)==np.round(median_bkg_only,1)):
            print("============= g_Z'^4 BANDS ==============\n")
            print("====== CENTRAL VALUE ========\n")
            print(gz4)
            plot_flag = True
            cl_sb = per95_signal_bkg
        if (np.round(upband_onesigma_bkg_only,1) == np.round(upband_onesigma_bkg_signal,1)):
            print(" + 1 sigma: \n")
            print(gz4)
            plot_flag = True
            cl_sb = upband_onesigma_bkg_signal
        if (np.round(upband_twosigma_bkg_only,2) == np.round(upband_twosigma_bkg_signal,2)):
            print(" + 2 sigma: \n")
            print(gz4)
            plot_flag = True
            cl_sb = upband_twosigma_bkg_signal
        if (np.round(lowband_onesigma_bkg_only,1) == np.round(lowband_onesigma_bkg_signal,1)):
            print(" - 1 sigma: \n")
            print(gz4)
            plot_flag = True
            cl_sb = lowband_onesigma_bkg_signal
        if (np.round(lowband_twosigma_bkg_only,1) == np.round(lowband_twosigma_bkg_signal,1)):
            print("- 2 sigma: \n")
            print(gz4)
            plot_flag = True
        ...
        if(plot_flag):
            plt.figure(dpi=300)
            plt.title(fr'$log(g_Z^4)$: {math.log(gz4):.2f} B: {b_cv:.0f}')
            ...
            for j in range(len(bins) - 1):
                if bins[j] >= vline:
                    ...
                    plt.fill_between([vline, bins[j+1]], 0, counts[j], color='red', alpha=1.,label = r'CL$_{s+b}= 0.05$')
