...
#print(str(path))
for i in range(4,8): #Each BDM sample gamma and mass value 
    ...
    Sensitivity_Info.write('ROUND_PRECISION: '+str(ROUND_PRECISION)+'\n')
    ...
    poi_m05 = np.linspace(1e-7,2e-6,STEPS_PROBING_GZ4)
    poi_m10 = np.linspace(1e-7,1.8e-6,STEPS_PROBING_GZ4)
    poi_m20 = np.linspace(1e-7,3e-6,STEPS_PROBING_GZ4)
    poi_m40 = np.linspace(5e-7,6e-6,STEPS_PROBING_GZ4)

    poi = [poi_m05, poi_m10, poi_m20, poi_m40]


    for gz4 in poi[i-4]: #Assumes the number of signal events
        ...
        Q_0 = np.float64(poisson.pmf(H_0, s_cv+b_cv))/np.float64(poisson.pmf(H_0, b_cv))
        Q_1 = np.float64(poisson.pmf(H_1, s_cv+b_cv))/np.float64(poisson.pmf(H_1, b_cv))
        nllr_h0 = np.minimum(100000., np.maximum(-100000., -2*np.log(np.float64(Q_0))))
        nllr_h1 = np.minimum(100000., np.maximum(-100000., -2*np.log(np.float64(Q_1))))
    ...
    ################################################################
        ...
        if (np.round(per95_signal_bkg,ROUND_PRECISION)==np.round(median_bkg_only,ROUND_PRECISION)):
            ...
            plot_flag = True
            cl_sb = per95_signal_bkg
        if (np.round(upband_onesigma_bkg_only,ROUND_PRECISION) == np.round(upband_onesigma_bkg_signal,ROUND_PRECISION)):
        ...
        if (np.round(upband_twosigma_bkg_only,ROUND_PRECISION) == np.round(upband_twosigma_bkg_signal,ROUND_PRECISION)):
            if (s_cv > 2):
        ...
        if (np.round(lowband_onesigma_bkg_only,ROUND_PRECISION) == np.round(lowband_onesigma_bkg_signal,ROUND_PRECISION)):
            Sensitivity_Info.write("======== -1 Sigma Expeceted BG ========\n")
            Sensitivity_Info.write(str(gz4)+'\n')
            print(" - 1 sigma: \n")
            print(gz4)
        ...
        if (np.round(lowband_twosigma_bkg_only,ROUND_PRECISION) == np.round(lowband_twosigma_bkg_signal,ROUND_PRECISION)):
            Sensitivity_Info.write("======== -2 Sigma Expeceted BG ========\n")
            Sensitivity_Info.write(str(gz4)+'\n')
            print("- 2 sigma: \n")
            print(gz4)
