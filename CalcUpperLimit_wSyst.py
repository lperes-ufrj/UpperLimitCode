...
def llh_best(effi,num_targets):
    return poisson.pmf(b,b)*norm.pdf(b,b,0.4*b)*norm.pdf(effi,eff,0.1*eff)*norm.pdf(num_targets,NA_dune,0.05*NA_dune)

def llh(nbkg, effi,num_targets, nevts):
    return poisson.pmf(nevts,nbkg)*norm.pdf(nbkg,b,0.4*b)*norm.pdf(effi,eff,0.1*eff)*norm.pdf(num_targets,NA_dune,0.05*NA_dune)

best_hyp_llh = nquad(llh_best, [[-1, 1.],[0.1*NA_dune,10*NA_dune]]) 


print(poisson.pmf(b,b)*norm.pdf(b,b,0.4*b)*norm.pdf(eff,eff,0.1*eff)*norm.pdf(NA_dune,NA_dune,0.05*NA_dune))
print(poisson.pmf(b,b)*norm.pdf(b,b,0.4*b))
print(best_hyp_llh) 
