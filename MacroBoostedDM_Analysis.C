...
void MakeWorkspace(double SigEff, double nBkg, double flux_value, double xsec_value, string nuclearModel, string sample)
{
   ...
   RooRealVar numtargets_kappa("numtargets_kappa", "numtargets_kappa", 1.05); // 5% of systematic unc. in the fiducial volume just as LBL analysis
   RooRealVar beta_numtargets("beta_numtargets", "beta_numtargets", 0, -5, 5);
   pWs->import(numtargets_nom);
   pWs->import(numtargets_kappa);
   pWs->import(beta_numtargets);
   pWs->factory("cexpr::alpha_numtargets('pow(numtargets_kappa,beta_numtargets)',numtargets_kappa,beta_numtargets)");
   pWs->factory("prod::numtargets(numtargets_nom,alpha_numtargets)");
   pWs->factory("Gaussian::constr_numtargets(beta_numtargets,glob_numtargets[0,-5,5],1)");

   // coupling constant (g_Z')^8 - parameter of interest
   RooRealVar gZ8("gZ8", "#g_{Z'}^{8}", 1e-15, 1e-15, gz8_samples[sample]);
   pWs->import(gZ8);
   ...
   RooRealVar eff_kappa("eff_kappa", "eff_kappa", 1.05);  // 5% of systematic unc. in the signal efficiency
   RooRealVar beta_eff("beta_eff", "beta_eff", 0, -5, 5);
   pWs->import(eff_nom);
   pWs->import(eff_kappa);
   pWs->import(beta_eff);

   pWs->factory("cexpr::alpha_eff('pow(eff_kappa,beta_eff)',eff_kappa,beta_eff)");
   pWs->factory("prod::eff(eff_nom,alpha_eff)");
   pWs->factory("Gaussian::constr_eff(beta_eff,glob_eff[0,-5,5],1)");
   ...
   RooRealVar nbkg_kappa("nbkg_kappa", "nbkg_kappa", 1.30); // 30% of systematic unc. in the atmospheric neutrino events expected
   RooRealVar beta_nbkg("beta_nbkg", "beta_nbkg", 0, -5, 5);
   pWs->import(nbkg_nom);
   pWs->import(nbkg_kappa);
   pWs->import(beta_nbkg);

   pWs->factory("cexpr::alpha_nbkg('pow(nbkg_kappa,beta_nbkg)',nbkg_kappa,beta_nbkg)");
   ...
   RooArgSet nuis("nuis");
   nuis.add(*pWs->var("beta_numtargets"));
   ...
   pWs->var("numtargets_kappa")->setConstant(true);
   ...
   fixed.add(*pWs->var("numtargets_kappa"));
